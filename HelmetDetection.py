...
def videoHelmetDetect():
    global filename
    video = cv.VideoCapture(00)
    #video = cv.VideoCapture(0) # for video detection using camera
    print('hiiiii')
    while(True):
        print('hi2222222222')
...
uploadimage.place(x=200,y=140)
uploadimage.config(font=font1) 

classifyimage = Button(main, text="Detect Helmet", command=detectHelmet)
classifyimage.place(x=200,y=180)
classifyimage.config(font=font1)


classifyimage = Button(main, text="Detect Helmet vedio", command=videoHelmetDetect)
classifyimage.place(x=200,y=220)
classifyimage.config(font=font1)



exitapp = Button(main, text="Exit", command=exit)
exitapp.place(x=200,y=260)
